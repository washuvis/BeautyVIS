...
import img10 from '../components/data/27.jpg';
import img11 from '../components/data/42.jpg';
...
class AreaChart extends Component {
    ...
    drawAreaChart() {
        ...
        const svg = d3.select("#graph_box")
            .append("svg")
            .attr("width", width + 3 * margin.left + 2 * margin.right)
        ...
        var image = svg.append('image').attr('width', width / 1.2).attr('x', 0).attr('y', margin.top * height / width).attr('xlink:href', img10).attr('height', 1.1 * height)
        var image2 = svg.append('image').attr('width', width / 1.2).attr('x', width / 1.2).attr('y', margin.top * height / width).attr('xlink:href', img11).attr('height', 1.1 * height)
        // var e = document.getElementById("graph_box");
        // const length = Math.min(e.clientHeight, e.clientWidth)
        ...
        //     .domain([new Date(2018, 1, 1), new Date(2019, 12, 1)])
        //     .range([0, width]);
